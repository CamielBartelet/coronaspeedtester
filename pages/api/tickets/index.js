...
myHeaders.append(
  "Cookie",
  "eventgoose_session=FOtUVItcJk7vDOOuZaLoIX7B4E4dqxR88f0M09MW; SERVERID=live-site1"
);
...
async function addToCart(eventId, ticketId) {
  ...
  const response = await fetch(
    "https://api.eventgoose.com/v1/checkouts",
    requestOptions
  );
...
async function postCustomer(sessionId, user) {
  var body = JSON.stringify({
    ...
    email: "renormapp@gmail.com",
    email_confirmation: "renormapp@gmail.com",
    phone: user.phone,
    accept_eg: true,
    fields: {
      general: {
  ...
  const response = await fetch(
    "https://api.eventgoose.com/v1/checkouts/" + sessionId + "/customer",
    requestOptions
  );
  const data = await response.json();
}
...
async function selectPaymentIssuer(sessionId, eventId, issuer) {
  ...
  const response = await fetch(
    "https://api.eventgoose.com/v1/checkouts/" + sessionId + "/payment-method",
    requestOptions
  );
  const data = await response.json();
